...
'''
This is the function used for visualizing road objects.
Only highlights relevant objects, ignores the rest
'''
def display_instances_10fps(image, boxes, masks, class_ids, class_names, 
    ...
    for i in range(N):
        ...
        # only highlight relevant road objects
        if class_ids[i] not in[1,2,3,4,6,7,8,10,11,12,13,14,17,18,19]: 
            continue

        # Bounding box - not displayed
        if not np.any(boxes[i]):
            continue
        y1, x1, y2, x2 = boxes[i]           # Get bounding box coords
        '''
        camera_color = (color[0] * 255, color[1] * 255, color[2] * 255)
        cv2.rectangle(masked_image, (x1, y1), (x2, y2), camera_color, 1)
        '''

        # if bus/car/truck/train is too far away, ignore
        if (class_ids[i] in [3,6] and abs(y2-y1)*abs(x1-x2) < 3600): 
            continue
        
        # if person is too far away, ignore
        if (class_ids[i] in [1] and abs(y2-y1)*abs(x1-x2) > 3000):
            person = 1

        # Check for stop signs
        if (class_ids[i] == 13 and stop_sign == 1):
            stop_sign = 1
    ...
    return image.astype(np.uint8), stop_sign, traffic_light, parking, person
